...
function IndexCardV4(props) {
  ...
  const previewFinalCanvas = whichColor => {
    ...
    drawFinalCanvas(
      ...
      finalCanvasRef,
      bgCanvasRef,
      [bgCanvasRef, pfpCanvasRef, textCanvasRef],
      fixedWidth,
      fixedHeight
    );
  ...
  const skipBgStep = () => {
    ...
    image.onload = () => {
      image = classifyIndexImage(
        image,
        getViewWidth(viewContainerRef),
        heightFromAspectRatio(getViewWidth(viewContainerRef), fixedWidth / fixedHeight)
      );
      setCroppedBg(image);
      ...
      <Box display="flex" style={{ flexFlow: "column", alignItems: "center" }}>
        ...
        {uiStep === "bg" && fileImage && (
          <Box className="two-btns-horizontal">
            ...
            <Button id="upload-pfp-button" variant="contained" className="index-button" onClick={goToPfpStep}>
              <Typography className="btn-text">Confirm</Typography>
            </Button>
          </Box>
        )}

        {(uiStep === "pfp" || uiStep === "text") && <ShareOnTwitter inIndexCard={true} />}
